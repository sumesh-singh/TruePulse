...
def perform_full_analysis(text, classifier, api_key, api_url, source_domain=None):
    ...
    reasoning.append(ai_result.get(
        'reasoning', 'The AI model provided an initial assessment.'))

    # Factor in the source domain if available
    if source_domain:
        if source_domain in TRUSTED_NEWS_DOMAINS:
            final_result['trust_score'] = min(
                100, ai_result.get('trust_score', 50) + 20)  # Boost score
            reasoning.append(
                f"The article is from a trusted source ({source_domain}), which strongly supports its authenticity.")
        else:
    ...
    if verification_result.get("verified_sources"):
        final_result['trust_score'] = min(
            # Big boost for verification
            100, final_result['trust_score'] + 25)
        reasoning.append(
            "Cross-verification found similar reports from other trusted news outlets, significantly increasing confidence in the story's authenticity.")
    else:
        final_result['trust_score'] = max(
            0, final_result['trust_score'] - 20)  # Penalize if no verification
        reasoning.append(
...
def analyze_unified():
    ...
    try:
        ...
        result = perform_full_analysis(
            text_to_analyze, classifier, api_key, api_url, source_domain)
